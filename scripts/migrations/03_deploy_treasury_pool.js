...
async function main() {
  ...
  const ltokenArguments = ["LDai Token", "LDai"];
  ...
  const lpRewardPerEpoch = hre.ethers.utils.parseEther("6000");
  const teamRewardPerEpoch = hre.ethers.utils.parseEther("1500");
  ...
  if (network === "mainnet") {
    ...
    dsecDistributionArguments = [10, 1641686400, 14 * 86400, 86400];
    lfiAddress = process.env.BSC_TESTNET_LFI_ADDRESS;
  ...
  console.log("LDai deployed to:", ltoken.address);
  console.log("DsecDistribution deployed to:", dsecDistribution.address);
  console.log("TreasuryPool deployed to:", treasuryPool.address);
